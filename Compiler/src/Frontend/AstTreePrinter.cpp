...
void ast_printer_print_expression_inner(Expression* expr, std::string prefix, std::string children_prefix) {
    ...
    case Expression::ExpressionKind::CALL: {
        printf("%sCall Expr\n", prefix.c_str());
        ast_printer_print_expression_inner(expr->call_exp.left, (children_prefix + T_JUNCTION).c_str(), (children_prefix + STRAIGHT).c_str());
        if (expr->call_exp.right)
            ast_printer_print_expression_inner(expr->call_exp.right, (children_prefix + L_JUNCTION).c_str(), (children_prefix + EMPTY).c_str());
    } break;
    case Expression::ExpressionKind::ACCESS: {
        printf("%sAccess Expr\n", prefix.c_str());
        ast_printer_print_expression_inner(expr->call_exp.left, (children_prefix + T_JUNCTION).c_str(), (children_prefix + STRAIGHT).c_str());
        ast_printer_print_expression_inner(expr->bin_exp.right, (children_prefix + L_JUNCTION).c_str(), (children_prefix + EMPTY).c_str());
    } break;
    ...
    case Expression::ExpressionKind::IDENT: {
        printf("%sIdent Node\n", prefix.c_str());
        printf("%s%s%s: %.*s\n", children_prefix.c_str(), L_JUNCTION, token_type_names[(uint16)expr->value.token->kind], (int)expr->value.token->length, expr->value.token->token);
    } break;
